...
export default function MedicalSupport() {
  ...
  const results = useMemo(() => items.filter(item => {
    ...
    const departmentMatches = department === "すべて" || item.departments.includes(department);
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              <div className="rounded-xl border border-amber-700/20 bg-amber-50 p-4 text-sm leading-7 text-amber-950">
                この検索は診断や医療機関の評価を行うものではありません。受診を希望する方が、東京都の公式オープンデータから候補を探すための機能です。診療内容・予約方法は受診前に電話または公式サイトでご確認ください。
              </div>
              ...
              <p className="text-sm text-foreground/65">収録範囲：都立病院、中央区、中野区。該当 {results.length}件</p>
            </CardContent>
          </Card>
            ...
            <div className="space-y-4">
              {results.slice(0, limit).map(item => <Card key={item.id}>
                <CardContent className="space-y-4 pt-6">
                  <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-xl font-bold">{item.name}</h2><p className="mt-1 text-sm text-foreground/65">{item.facilityType}・{item.area}</p></div><span className="rounded-full bg-green-700 px-3 py-1 text-xs font-bold text-white">東京都オープンデータ</span></div>
